...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    def save_latest(epoch):
        ...
        save_model(experiment_directory, 'latest_direct.pth', decoder_direct, epoch)
        save_latent_vectors(experiment_directory, 'latest_direct.pth', direct_lat_vecs, epoch)
    def save_checkpoints(epoch):
        ...
        save_model(experiment_directory, str(epoch) + "_direct.pth", decoder_direct, epoch)
        save_latent_vectors(experiment_directory, str(epoch) + '_direct.pth', direct_lat_vecs, epoch)
    ...
    def adjust_learning_rate(lr_schedules, optimizer, epoch):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            k = i if i <=1 else 0
            param_group["lr"] = lr_schedules[k].get_learning_rate(epoch)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices in sdf_loader:
            ...
            for i in range(batch_split):
                ...
                chunk_loss = loss_l1(pred_sdf, sdf_gt[i].cuda()) / num_sdf_samples + 0.01 * loss_l1(pred_direct, sdf_direct[i].cuda()) / num_sdf_samples
                ...
                with torch.no_grad():
                    chunk_loss_sdf = (loss_l1(pred_sdf, sdf_gt[i].cuda()) / num_sdf_samples + reg_loss.cuda()).item()
                    chunk_loss_direct = (loss_l1(pred_direct, sdf_direct[i].cuda()) / num_sdf_samples + reg_loss_direct.cuda()).item()
