...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    def save_checkpoints(epoch):
        ...
        save_model(experiment_directory, str(epoch) + "decoder_direct.pth", decoder_direct, epoch)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices in sdf_loader:
            ...
            for i in range(batch_split):
                ...
                pred_direct = decoder_direct(input)
                if enforce_minmax:
                    pred_sdf = torch.clamp(pred_sdf, minT, maxT)

                chunk_loss = loss_l1(pred_sdf, sdf_gt[i].cuda()) / num_sdf_samples + loss_l1(pred_direct, sdf_direct[i].cuda()) / num_sdf_samples
