...
void SampleSDFNearSurface(
    ...
    std::vector<Eigen::Vector3f>& xyz_direction,
  ...
  for (int s = 0; s < (int)xyz.size(); s++) {
    ...
    if ((num_pos == 0) || (num_pos == num_votes)) {
      ...
      sdfs.push_back(sdf);
      xyz_direction.push_back(direct_vec);
    }
  }
...
void writeSDFToNPY(
  ...
  std::vector<float> data(num_vert * 7);
  int data_i = 0;

  for (unsigned int i = 0; i < num_vert; i++) {
    Eigen::Vector3f v = xyz[i];
    Eigen::Vector3f direct_vec = xyz_direction[i];
    float s = sdfs[i];
    ...
    data[data_i++] = s;
    for (int j = 0; j < 3; j++)
      data[data_i++] = direct_vec[j];
  }

  cnpy::npy_save(filename, &data[0], {(long unsigned int)num_vert, 7}, "w");
}
...
void writeSDFToNPZ(
  ...
  for (unsigned int i = 0; i < num_vert; i++) {
    ...
    if (s > 0) {
      ...
      pos.push_back(s);
      for (int j = 0; j < 3; j++)
        pos.push_back(direct_vec[j]);
    } else {
      for (int j = 0; j < 3; j++)
        neg.push_back(v[j]);
      neg.push_back(s);
      for (int j = 0; j < 3; j++)
        neg.push_back(direct_vec[j]);
    }
  }

  cnpy::npz_save(filename, "pos", &pos[0], {(long unsigned int)(pos.size() / 7.0), 7}, "w");
  cnpy::npz_save(filename, "neg", &neg[0], {(long unsigned int)(neg.size() / 7.0), 7}, "a");
  if (print_num) {
    std::cout << "pos num: " << pos.size() / 7.0 << std::endl;
    std::cout << "neg num: " << neg.size() / 7.0 << std::endl;
  }
}
...
void writeSDFToPLY(
  ...
  for (int i = 0; i < (int)sdfs.size(); i++) {
    Eigen::Vector3f v = xyz[i];
    Eigen::Vector3f d = xyz_direction[i];
    float sdf = sdfs[i];
    bool neg = (sdf <= 0);
    bool pos = (sdf >= 0);
    if (neg)
    ...
    if (!neg_only && pos)
      plyFile << v[0] << " " << v[1] << " " << v[2] << " " << 0 << " " << 0 << " " << sdf_i << d[0] << d[1] << d[2] << "\n";
    if (!pos_only && neg)
      plyFile << v[0] << " " << v[1] << " " << v[2] << " " << sdf_i << " " << 0 << " " << 0 << d[0] << d[1] << d[2] << "\n";
  }
  plyFile.close();
}
...
int main(int argc, char** argv) {
  ...
  if (vis)
    ...
    pangolin::CreateWindowAndBind("Man", 1, 1);
...
//  // Create Framebuffer with attached textures
  ...
  std::cout<< point_verts.size() << std::endl;
  ...
  if (save_ply) {
    writeSDFToPLY(xyz, sdf, xyz_direction, plyFileNameOut, false, true);
  }

  std::cout << "num points sampled: " << xyz.size() << std::endl;
  std::size_t save_npz = npyFileName.find("npz");
  if (save_npz == std::string::npos)
    writeSDFToNPY(xyz, sdf, xyz_direction, npyFileName);
  else {
    writeSDFToNPZ(xyz, sdf, xyz_direction, npyFileName, true);
  }
